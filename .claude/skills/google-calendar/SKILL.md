...
allowed-tools: Bash(AIOS_DIR=* ~/.venv/bin/python3 ~/.config/calendar/gcal.py *)
---
...
The calendar CLI reads credential paths from environment variables, loaded from the AIOS `.env` via `AIOS_DIR`. Every command MUST use this prefix:
...
```bash
AIOS_DIR="/opt/aios" ~/.venv/bin/python3 ~/.config/calendar/gcal.py list
```
...
```bash
AIOS_DIR="/opt/aios" ~/.venv/bin/python3 ~/.config/calendar/gcal.py list --days 7
```

### Get event details

```bash
AIOS_DIR="/opt/aios" ~/.venv/bin/python3 ~/.config/calendar/gcal.py get <event_id>
```

### Create event with Meet link and invites

```bash
AIOS_DIR="/opt/aios" ~/.venv/bin/python3 ~/.config/calendar/gcal.py create "Meeting Title" "2026-03-15 10:00" --duration 30 --attendees "person@example.com,other@example.com" --meet
```
...
```bash
AIOS_DIR="/opt/aios" ~/.venv/bin/python3 ~/.config/calendar/gcal.py update <event_id> --title "New Title" --start "2026-03-16 14:00" --duration 60 --add-attendees "new@example.com" --meet
```
...
```bash
AIOS_DIR="/opt/aios" ~/.venv/bin/python3 ~/.config/calendar/gcal.py cancel <event_id>
```
...
```bash
AIOS_DIR="/opt/aios" ~/.venv/bin/python3 ~/.config/calendar/gcal.py freebusy "2026-03-15 09:00" "2026-03-15 17:00"
```
...
```bash
AIOS_DIR="/opt/aios" ~/.venv/bin/python3 ~/.config/calendar/gcal.py auth
```
